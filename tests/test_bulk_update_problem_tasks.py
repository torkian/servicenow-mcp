"""Tests for bulk_update_problem_tasks in bulk_tools.py."""
...
class TestBulkUpdateProblemTasksValidation(unittest.TestCase):
    def test_empty_updates_returns_failure(self):
        params = BulkUpdateProblemTasksParams.__new__(BulkUpdateProblemTasksParams)
    ...
    def test_over_100_updates_returns_failure(self):
        params = BulkUpdateProblemTasksParams.__new__(BulkUpdateProblemTasksParams)
        too_many = [
            ProblemTaskUpdate(task_id=_SYS_ID_A, short_description=f"Update {i}")
            for i in range(101)
        ]
...
class TestBulkUpdateProblemTasksAllSysIds(unittest.TestCase):
    ...
    def test_single_sys_id_update_success(self, mock_post):
        ...
        params = BulkUpdateProblemTasksParams(
            updates=[ProblemTaskUpdate(task_id=_SYS_ID_A, state="2")]
        )
        result = bulk_update_problem_tasks(_config(), _auth(), params)
    ...
    def test_multiple_sys_ids_patch_url_contains_sys_id(self, mock_post):
        ...
        params = BulkUpdateProblemTasksParams(
            updates=[
                ProblemTaskUpdate(task_id=_SYS_ID_A, state="2"),
                ProblemTaskUpdate(task_id=_SYS_ID_B, state="3"),
            ]
        )
        bulk_update_problem_tasks(_config(), _auth(), params)

        payload = mock_post.call_args[1]["json"]
        urls = [r["url"] for r in payload["requests"]]
        self.assertIn(f"/api/now/v2/table/problem_task/{_SYS_ID_A}", urls)
        self.assertIn(f"/api/now/v2/table/problem_task/{_SYS_ID_B}", urls)
    ...
    def test_patch_method_used_in_batch(self, mock_post):
        ...
        params = BulkUpdateProblemTasksParams(
            updates=[ProblemTaskUpdate(task_id=_SYS_ID_A, work_notes="investigating")]
        )
        bulk_update_problem_tasks(_config(), _auth(), params)
    ...
    def test_only_provided_fields_in_body(self, mock_post):
        ...
        params = BulkUpdateProblemTasksParams(
            updates=[
                ProblemTaskUpdate(task_id=_SYS_ID_A, state="2", priority="1")
            ]
        )
        bulk_update_problem_tasks(_config(), _auth(), params)
        ...
        self.assertEqual(body, {"state": "2", "priority": "1"})
...
class TestBulkUpdateProblemTasksNumberResolution(unittest.TestCase):
    """Tests covering the PTASK number → sys_id resolution path."""

    @patch("servicenow_mcp.tools.bulk_tools.requests.post")
    @patch("servicenow_mcp.tools.bulk_tools.requests.get")
    def test_ptask_number_resolved_before_batch(self, mock_get, mock_post):
        mock_get.return_value = _get_response(
            [{"number": "PTASK0010001", "sys_id": _SYS_ID_A}]
        )
        mock_post.return_value = _batch_response(
            [{"id": "0", "statusCode": 200, "statusText": "OK", "body": "{}"}]
        )
        params = BulkUpdateProblemTasksParams(
            updates=[ProblemTaskUpdate(task_id="PTASK0010001", state="2")]
        )
        result = bulk_update_problem_tasks(_config(), _auth(), params)
    ...
    @patch("servicenow_mcp.tools.bulk_tools.requests.post")
    @patch("servicenow_mcp.tools.bulk_tools.requests.get")
    def test_resolution_query_targets_problem_task_table(self, mock_get, mock_post):
        mock_get.return_value = _get_response(
            [{"number": "PTASK0010001", "sys_id": _SYS_ID_A}]
        )
        mock_post.return_value = _batch_response(
        ...
        self.assertIn("problem_task", get_url)
    ...
    def test_resolution_query_uses_numberIN_operator(self, mock_get, mock_post):
        mock_get.return_value = _get_response(
            [
                {"number": "PTASK0010001", "sys_id": _SYS_ID_A},
                {"number": "PTASK0010002", "sys_id": _SYS_ID_B},
            ]
        )
        mock_post.return_value = _batch_response(
            [
        ...
        params = BulkUpdateProblemTasksParams(
            updates=[
                ProblemTaskUpdate(task_id="PTASK0010001", state="2"),
                ProblemTaskUpdate(task_id="PTASK0010002", priority="1"),
            ]
        )
        bulk_update_problem_tasks(_config(), _auth(), params)
        ...
        self.assertIn("PTASK0010001", query)
        self.assertIn("PTASK0010002", query)
    ...
    def test_single_get_issued_for_all_numbers(self, mock_get, mock_post):
        ...
        mock_get.return_value = _get_response(
            [{"number": f"PTASK000{i}", "sys_id": ids[i]} for i in range(3)]
        )
        mock_post.return_value = _batch_response(
            [
                {"id": str(i), "statusCode": 200, "statusText": "OK", "body": "{}"}
        ...
        params = BulkUpdateProblemTasksParams(
            updates=[
                ProblemTaskUpdate(task_id=f"PTASK000{i}", state="2") for i in range(3)
            ]
        )
        bulk_update_problem_tasks(_config(), _auth(), params)
    ...
    def test_unresolved_number_returns_failure(self, mock_get):
        mock_get.return_value = _get_response([])
        params = BulkUpdateProblemTasksParams(
            updates=[ProblemTaskUpdate(task_id="PTASK0099999", state="2")]
        )
        result = bulk_update_problem_tasks(_config(), _auth(), params)

        self.assertFalse(result["success"])
        self.assertIn("PTASK0099999", result["message"])
        self.assertIn("PTASK0099999", result.get("unresolved", []))
    ...
    def test_resolution_network_error_returns_failure(self, mock_get):
        ...
        self.assertFalse(result["success"])
        self.assertIn("resolve problem task numbers", result["message"])

    @patch("servicenow_mcp.tools.bulk_tools.requests.post")
    @patch("servicenow_mcp.tools.bulk_tools.requests.get")
    def test_mixed_sys_ids_and_numbers(self, mock_get, mock_post):
        mock_get.return_value = _get_response(
            [{"number": "PTASK0010001", "sys_id": _SYS_ID_B}]
        )
        mock_post.return_value = _batch_response(
            [
                {"id": "0", "statusCode": 200, "statusText": "OK", "body": "{}"},
        ...
        params = BulkUpdateProblemTasksParams(
            updates=[
                ProblemTaskUpdate(task_id=_SYS_ID_A, state="2"),
                ProblemTaskUpdate(task_id="PTASK0010001", priority="1"),
            ]
        )
        result = bulk_update_problem_tasks(_config(), _auth(), params)
    ...
    @patch("servicenow_mcp.tools.bulk_tools.requests.post")
    @patch("servicenow_mcp.tools.bulk_tools.requests.get")
    def test_result_entries_carry_task_id_for_number(self, mock_get, mock_post):
        mock_get.return_value = _get_response(
            [{"number": "PTASK0010001", "sys_id": _SYS_ID_A}]
        )
        mock_post.return_value = _batch_response(
        ...
        self.assertEqual(result["results"][0]["task_id"], "PTASK0010001")
    ...
    @patch("servicenow_mcp.tools.bulk_tools.requests.post")
    def test_close_notes_and_work_notes_included(self, mock_post):
        mock_post.return_value = _batch_response(
            [{"id": "0", "statusCode": 200, "statusText": "OK", "body": "{}"}]
        )
        params = BulkUpdateProblemTasksParams(
            updates=[
                ProblemTaskUpdate(
                    task_id=_SYS_ID_A,
                    close_notes="resolved via patch",
                    work_notes="applied fix",
                )
            ]
        )
        bulk_update_problem_tasks(_config(), _auth(), params)
        ...
        payload = mock_post.call_args[1]["json"]
        body = json.loads(payload["requests"][0]["body"])
        self.assertEqual(body["close_notes"], "resolved via patch")
        self.assertEqual(body["work_notes"], "applied fix")
