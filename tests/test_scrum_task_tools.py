...
class TestScrumTaskTools(unittest.TestCase):
    ...
    def test_create_scrum_task(self, mock_post):
        mock_post.return_value = MagicMock(
            ...
            json=MagicMock(
                return_value={"result": {"sys_id": "st1", "short_description": "Task1"}}
            ),
        )
        result = create_scrum_task(
            self.auth,
            self.config,
            {
                "story": "s1",
                "short_description": "Task1",
            },
        )
    ...
    def test_create_scrum_task_error(self, mock_post):
        ...
        result = create_scrum_task(
            self.auth, self.config, {"story": "s1", "short_description": "T"}
        )
