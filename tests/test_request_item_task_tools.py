"""Tests for request_item_task_tools (create, list, close request item tasks)."""
...
SAMPLE_TASK_RECORD = {
    ...
    "short_description": "Configure software",
    "description": "Install and configure required software",
    "state": "1",
    "priority": "3",
    "assigned_to": {"display_value": "john.smith", "value": "c" * 32},
    "assignment_group": {"display_value": "Desktop Support", "value": "d" * 32},
    "request_item": {"display_value": "RITM0010001", "value": RITM_SYS_ID},
    "close_notes": None,
    "sys_created_on": "2026-07-19 09:00:00",
    "sys_updated_on": "2026-07-19 09:00:00",
}

CLOSED_TASK_RECORD = {
    **SAMPLE_TASK_RECORD,
    "state": "3",
    "close_notes": "Software installed successfully",
}
...
class TestCreateRequestItemTask(unittest.TestCase):

    @patch("servicenow_mcp.tools.request_item_task_tools._make_request")
    def test_create_success_by_sys_id(self, mock_req):
        """Create a task when item_id is already a sys_id (no lookup needed)."""
        mock_req.return_value = _make_response(201, {"result": SAMPLE_TASK_RECORD})

        result = create_request_item_task(
            _make_auth_manager(),
            _make_config(),
            {
                "item_id": RITM_SYS_ID,
        ...
        self.assertIn("SCTASK0010001", result["message"])
        self.assertEqual(result["sys_id"], TASK_SYS_ID)
        self.assertEqual(result["number"], "SCTASK0010001")
        self.assertEqual(result["task"]["assigned_to"], "john.smith")
        self.assertEqual(result["task"]["assignment_group"], "Desktop Support")
        self.assertEqual(result["task"]["request_item"], "RITM0010001")
        mock_req.assert_called_once()
        body = mock_req.call_args[1]["json"]
        self.assertEqual(body["request_item"], RITM_SYS_ID)

    @patch("servicenow_mcp.tools.request_item_task_tools._make_request")
    def test_create_success_by_ritm_number(self, mock_req):
        """Create a task using a RITM number; lookup resolves to sys_id first."""
        lookup_resp = _make_response(200, {"result": [{"sys_id": RITM_SYS_ID}]})
    ...
    def test_create_with_all_optional_fields(self, mock_req):
        ...
        mock_req.side_effect = [
            _make_response(200, {"result": [{"sys_id": RITM_SYS_ID}]}),
        ...
        self.assertEqual(body["assigned_to"], "john.smith")
        self.assertEqual(body["assignment_group"], "Desktop Support")
        self.assertEqual(body["priority"], "3")
        self.assertEqual(body["state"], "2")
        self.assertEqual(body["work_notes"], "Starting work")

    @patch("servicenow_mcp.tools.request_item_task_tools._make_request")
    def test_create_ritm_not_found(self, mock_req):
        """Return failure when the RITM number resolves to nothing."""
        mock_req.return_value = _make_response(200, {"result": []})

        result = create_request_item_task(
            _make_auth_manager(),
            _make_config(),
            {"item_id": "RITM9999999", "short_description": "Task for missing RITM"},
        )
    ...
    def test_create_missing_required_params(self):
        ...
        result = create_request_item_task(_make_auth_manager(), _make_config(), {})
        self.assertFalse(result["success"])

    def test_create_missing_short_description(self):
        """Missing short_description should fail validation."""
        result = create_request_item_task(
            _make_auth_manager(), _make_config(), {"item_id": "RITM0010001"}
        )
        self.assertFalse(result["success"])
        self.assertIn("short_description", result["message"])

    def test_create_missing_item_id(self):
        """Missing item_id should fail validation."""
        result = create_request_item_task(
            _make_auth_manager(),
            _make_config(),
            {"short_description": "Task without RITM"},
        )
        self.assertFalse(result["success"])
        self.assertIn("item_id", result["message"])
    ...
    def test_create_api_error(self, mock_req):
        ...
        result = create_request_item_task(
            _make_auth_manager(),
            _make_config(),
            {"item_id": RITM_SYS_ID, "short_description": "Task"},
        )

        self.assertFalse(result["success"])
        self.assertIn("Error creating request item task", result["message"])

    @patch("servicenow_mcp.tools.request_item_task_tools._make_request")
    def test_create_lookup_exception(self, mock_req):
        """ConnectionError during RITM lookup returns failure."""
        mock_req.side_effect = requests.exceptions.ConnectionError("timeout")

        result = create_request_item_task(
            _make_auth_manager(),
            _make_config(),
            {"item_id": "RITM0010001", "short_description": "Task"},
        )
...
class TestListRequestItemTasks(unittest.TestCase):

    @patch("servicenow_mcp.tools.request_item_task_tools._make_request")
    def test_list_success_by_sys_id(self, mock_req):
        """List tasks when item_id is a sys_id (no lookup needed)."""
        mock_req.return_value = _make_response(
            200, {"result": [SAMPLE_TASK_RECORD, {**SAMPLE_TASK_RECORD, "number": "SCTASK0010002"}]}
        )

        result = list_request_item_tasks(
            _make_auth_manager(),
            _make_config(),
            {"item_id": RITM_SYS_ID},
        )
    ...
    @patch("servicenow_mcp.tools.request_item_task_tools._make_request")
    def test_list_success_by_ritm_number(self, mock_req):
        """List tasks using a RITM number; lookup resolves to sys_id first."""
        lookup_resp = _make_response(200, {"result": [{"sys_id": RITM_SYS_ID}]})
    ...
    def test_list_pagination(self, mock_req):
        ...
        result = list_request_item_tasks(
            _make_auth_manager(),
            _make_config(),
            {"item_id": RITM_SYS_ID, "limit": 5, "offset": 0},
        )
    ...
    @patch("servicenow_mcp.tools.request_item_task_tools._make_request")
    def test_list_ritm_not_found(self, mock_req):
        """Return failure when the RITM number resolves to nothing."""
        mock_req.return_value = _make_response(200, {"result": []})
    ...
    def test_list_missing_item_id(self):
        """Missing item_id should fail validation."""
        result = list_request_item_tasks(_make_auth_manager(), _make_config(), {})
        self.assertFalse(result["success"])
        self.assertIn("item_id", result["message"])
    ...
    def test_list_api_error(self, mock_req):
        ...
        self.assertFalse(result["success"])
        self.assertIn("Error listing request item tasks", result["message"])

    @patch("servicenow_mcp.tools.request_item_task_tools._make_request")
    def test_list_query_includes_ritm_sys_id(self, mock_req):
        """sysparm_query must contain the resolved RITM sys_id."""
        mock_req.return_value = _make_response(200, {"result": []})
        ...
        self.assertIn(RITM_SYS_ID, params["sysparm_query"])

    @patch("servicenow_mcp.tools.request_item_task_tools._make_request")
    def test_list_normalises_reference_fields(self, mock_req):
        """assigned_to, assignment_group, and request_item are normalised to display values."""
        ...
        self.assertEqual(task["assignment_group"], "Desktop Support")
        self.assertEqual(task["request_item"], "RITM0010001")
...
class TestCloseRequestItemTask(unittest.TestCase):
    ...
    def test_close_by_sys_id_skips_lookup(self, mock_req):
        ...
        result = close_request_item_task(
            _make_auth_manager(),
            _make_config(),
            {"task_id": TASK_SYS_ID},
        )
    ...
    def test_close_by_task_number(self, mock_req):
        """Close a task using its SCTASK number; lookup resolves to sys_id."""
    ...
    def test_close_with_close_notes(self, mock_req):
        ...
        result = close_request_item_task(
            _make_auth_manager(),
            _make_config(),
            {"task_id": "SCTASK0010001", "close_notes": "Software installed successfully"},
        )

        self.assertTrue(result["success"])
        body = mock_req.call_args[1]["json"]
        self.assertEqual(body["close_notes"], "Software installed successfully")
    ...
    def test_close_with_work_notes(self, mock_req):
        ...
        result = close_request_item_task(
            _make_auth_manager(),
            _make_config(),
            {"task_id": "SCTASK0010001", "work_notes": "Closing after verification"},
        )

        self.assertTrue(result["success"])
        body = mock_req.call_args[1]["json"]
        self.assertEqual(body["work_notes"], "Closing after verification")
    ...
    def test_close_missing_task_id(self):
        ...
        result = close_request_item_task(_make_auth_manager(), _make_config(), {})
        self.assertFalse(result["success"])
    ...
    def test_close_api_error(self, mock_req):
        ...
        self.assertFalse(result["success"])
        self.assertIn("Error closing request item task", result["message"])
    ...
    def test_close_returns_task_fields(self, mock_req):
        ...
        self.assertEqual(task["close_notes"], "Software installed successfully")
        self.assertEqual(task["assigned_to"], "john.smith")
