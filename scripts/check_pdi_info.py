...
def check_instance_info(instance_url):
    ...
    try:
        response = requests.get(
            f"{instance_url}/api/now/table/sys_properties?sysparm_limit=1",
...
def update_env_file(instance_url, username, password):
    ...
    env_content = env_content.replace(
        f"SERVICENOW_INSTANCE_URL={os.getenv('SERVICENOW_INSTANCE_URL', 'https://your-instance.service-now.com')}",
        f"SERVICENOW_INSTANCE_URL={instance_url}",
    )
    env_content = env_content.replace(
        f"SERVICENOW_USERNAME={os.getenv('SERVICENOW_USERNAME', 'your-username')}",
        f"SERVICENOW_USERNAME={username}",
    )
    env_content = env_content.replace(
        f"SERVICENOW_PASSWORD={os.getenv('SERVICENOW_PASSWORD', 'your-password')}",
        f"SERVICENOW_PASSWORD={password}",
    )
