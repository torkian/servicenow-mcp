...
def check_instance_status(instance_url):
    ...
    try:
        ...
        login_response = requests.get(f"{instance_url}/login.do", allow_redirects=True, timeout=10)
        ...
        if (
            "instance is hibernating" in login_response.text.lower()
            or "instance is hibernating" in api_response.text.lower()
        ):
