...
def setup_api_key():
    ...
    if not instance_url or instance_url == "https://your-instance.service-now.com":
        instance_url = input(
            "Enter your ServiceNow instance URL (e.g., https://dev296866.service-now.com): "
        )
    ...
    api_key = input("\nEnter your API key: ")
    api_key_header = (
        input("Enter the API key header name (default: X-ServiceNow-API-Key): ")
        or "X-ServiceNow-API-Key"
    )
    ...
    try:
        ...
        test_response = requests.get(
            test_url, headers={api_key_header: api_key, "Accept": "application/json"}
        )
        ...
        if test_response.status_code == 200:
            ...
            if update_env.lower() == "y":
                ...
                env_content = env_content.replace(
                    "SERVICENOW_AUTH_TYPE=basic", "SERVICENOW_AUTH_TYPE=api_key"
                )
                env_content = env_content.replace(
                    "# SERVICENOW_API_KEY=your-api-key", f"SERVICENOW_API_KEY={api_key}"
                )
                env_content = env_content.replace(
                    "# SERVICENOW_API_KEY_HEADER=X-ServiceNow-API-Key",
                    f"SERVICENOW_API_KEY_HEADER={api_key_header}",
                )
